...
public class CardScreen_Equipped_Weapon : MonoBehaviour
{
    ...
    void Update()
    {
        if (cards.Count != data.bd.WeaponCardList.Count)
        {
            cards = new List<int>(data.bd.WeaponCardList);
            DisplayCard();
        }
    ...
    void DisplayCard()
    {
        ...
        for (int i = 0; i < template.Count; i++)
        {
            if (i < data.bd.WeaponCardList.Count)
            {
                ...
                c.GetComponent<CardScreen_EquippedCard>().cid = data.bd.WeaponCardList[i];
                displayList.Add(c);
                c.SetActive(true);
            }
            else
